...
exports.register = function (server, options, next) {
    ...
    function alias(url) {
        ...
        for (let alias in settings.aliases) {
            url = url.replace(alias, settings.aliases[alias]);
        }
    ...
    server.route({ method: 'GET', path: `/${settings.url}/{param*}`, handler(request, reply) {
        ...
        if (request.headers.referer) {
            ...
            const componentAssetsLocation = alias(join(settings.baseDir, componentFromReferer, settings.location));
        ...
        const staticAssetsLocation = alias(join(settings.baseDir, settings.location));
